...
class EstimateBaApp:
    def __init__(self):
        ...
        self.brake = st.radio('是否设置停留刹车', ('是', '否'))
    ...
    def calculate(self):
        if self.aircraft_model == '737-800':
            ...
            center_side_fuel_volume_new = float(self.center_side_fuel_volume_new) / fuel_density
            crew_weight = float(self.crew_weight) * 2.20462262185
            add_weight = float(self.add_weight)
            additional_moment = float(self.additional_moment)
            brake = self.brake == '是'
            cg_new = ((index - 45) * 77162 / gw_new + 658.3 - 627.1) / 1.558
            friction_coefficient = float(self.friction_coefficient)
            total_weight, cg_position_new = calculate_weight_balance(self.aircraft_model, fuel_density, gw_new,
                                                                     cg_new, left_side_fuel_volume_new,
                                                                     center_side_fuel_volume_new, crew_weight,
                                                                     add_weight, additional_moment)
            if total_weight is None or cg_position_new is None:
                st.write("计算过程中出现错误")
                return

            mw_new = estimate_ba(cg_position_new, total_weight / 1000, brake, friction_coefficient, self.aircraft_model)
            if mw_new is None:
                st.write("计算过程中出现错误")
                return
